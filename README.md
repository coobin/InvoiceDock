...
> 
> 🌐 **在线体验地址**：[https://fapiao.okkay.tech](https://fapiao.okkay.tech)
...
- 待开票预录入与核销闭环、分项目归集与预算进度监控（支持发票关联项目、台账导出项目明细与预录台账）。
...
- WebAuthn / FIDO2 Passkey（通行密钥）免用户名免密码设备识别登录（支持 Touch ID、Face ID、Windows Hello、安全密钥）。
- 邮箱/密码登录智能防撞库保护（连续失败 2 次动态激活本地矢量 SVG 验证码），可选 OIDC 登录，登录与注册限流，角色控制，CSRF 防护，敏感配置加密存储，审计日志。
