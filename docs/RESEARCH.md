...
没有发现同时覆盖全部目标且许可清晰、仍提供完整源码的项目。最接近的开源基线是 `ke4king/invoice_system`，但它使用百度 OCR 和本地 JWT，不含税务优先查验、OIDC 和 OCR/LLM 证据对照。本项目因此采用原创实现，并只借鉴公开产品思路。
...
| [ke4king/invoice_system](https://github.com/ke4king/invoice_system) | FastAPI + Vue、邮箱、百度 OCR、1/2/4 联、Docker | MIT，功能最接近；缺税务、OIDC、双源证据语义 |
...
- 选择“税务已查验 / 双源一致 / 人工已复核”三种不同状态，不使用单一“验证成功”。
